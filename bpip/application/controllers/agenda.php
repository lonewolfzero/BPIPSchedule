...
class Agenda extends CI_controller
{
  ...
  public function index2()
  {
    ...
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($id) : $this->m_admin->agenda_pegawai();
	$data3 = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($idm2) : $this->m_admin->cari_agenda_pegawai($idm2);
	$data2 = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawaim2($idm2) : $this->m_admin->cari_agenda_pegawaim2($idm2);
	...
	{
	   $idman=$dataman['id_agenda'];
	   $dataman=$this->db->query("SELECT * from agenda WHERE id_agenda='$idman'")->row();
  ...
  public function myagendacalendar()
  {
    ...
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($idm2) : $this->m_admin->cari_agenda_pegawai($idm2);
	$data2 = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawaim2($idm2) : $this->m_admin->cari_agenda_pegawaim2($idm2);
	$data3 = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($idm2) : $this->m_admin->agenda_pegawai();
	...
	{
      tpl('admin/myagendacalendar',$x);
	}
	else
	{
	  tpl('admin/mysagendacalendar',$x);
	}
  ...
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($id) : $this->m_admin->agenda_pegawai_disposisi();
  ...
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->kegiatan_pegawai($id_agenda) : $this->m_admin->kegiatan_pegawai($id_agenda);
	
	$sql=$this->db->get_where('agenda',array('id_agenda'=>$id_agenda))->row_array(); 
  ...
  public function verifikasiagenda_kegiatan($id_agenda)
  {
    ...
    $x = array('judul' =>'Daftar Kegiatan Agenda Pegawai',
	           'idm'=>$id_agenda,
				  ...
				  'lokasi_agenda'    => $sql['lokasi_agenda'],
              'data'  =>$data); 
    tpl('admin/verifikasiagendakegiatan',$x);
  }
  ...
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->disposisikegiatan_pegawai($id_agenda) : $this->m_admin->disposisikegiatan_pegawai($id_agenda);
    ...
    $x = array('judul' =>'Disposisi Daftar Kegiatan Agenda Pegawai',
	           'idm'=>$id_agenda,
				  ...
				  'lokasi_agenda'    => $sql['lokasi_agenda'],
              'data'  =>$data); 
    tpl('admin/disposisiagendakegiatan',$x);
  }
  ...
  public function setujui_kegiatan($id,$idm)
  {
	    ...
	    if($sql['status_kegiatan']==0)
		{
		  ...
		  redirect(base_url('agenda/verifikasiagenda_kegiatan/'.$idm));
		}
		else
		{
		  redirect(base_url('agenda/verifikasiagenda_kegiatan/'.$idm));
  ...
  public function tolak_kegiatan($id='',$idm='')
  {
		...
		{   	  
			  $x = array('judul'        =>'Tolak Data Kegiatan' ,
				...
				{
				 tpl('admin/tolakkegiatan_form',$x);
				}
		...
		{   	  
			  if($sql['status_kegiatan']==0)
			  {
			   ...
			   else
			   {
				...
				{
				 tpl('admin/wakilkegiatan_form',$x);
				}
  ...
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($id) : $this->m_admin->agenda_pegawai_pending();
    $x = array('judul' =>'Pending Agenda Pegawai',
              'data'  =>$data); 
    tpl('admin/agendapending',$x);
  }
  
  public function agenda_approved()
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($id) : $this->m_admin->agenda_pegawai_approved();
    $x = array('judul' =>'Apporved Agenda Pegawai',
              'data'  =>$data); 
    tpl('admin/agendaapproved',$x);
  }
  
  public function agenda_reject()
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($id) : $this->m_admin->agenda_pegawai_reject();
    $x = array('judul' =>'Reject Agenda Pegawai',
              'data'  =>$data); 
    tpl('admin/agendareject',$x);
  }
  ...
  public function agenda_tambah()
  {
	  $x = array('judul'        => 'Tambah Data Agenda' ,
		...
		if(isset($_POST['kirim']))
		{
			...
			$querydatajumdata=0;
			$querydatajum = $this->db->query("SELECT * FROM agenda WHERE id_pegawai='$idpegawai' AND tanggal='$tanggal' AND (jam_mulai >= '$jammulai2' AND jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') ");
			$querydatajumdata=$querydatajum->num_rows();
			$querydatajumdatarow=$querydatajum->row();

			$querydatajum21 = $this->db->query("SELECT * FROM agendapegawai a INNER JOIN agenda b ON(a.id_agenda=b.id_agenda) WHERE a.id_pegawai='$idpegawai' AND b.tanggal='$tanggal' AND (b.jam_mulai >= '$jammulai2' AND b.jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2')");
			...
			if(!empty($datapegawai))
			{
				foreach($datapegawai as $datapegikut)
				{
					...
					$querydatajum2 = $this->db->query("SELECT * FROM agendapegawai a INNER JOIN agenda b ON(a.id_agenda=b.id_agenda) WHERE a.id_pegawai='$datapegikut' AND b.tanggal='$tanggal' AND (b.jam_mulai >= '$jammulai2' AND b.jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2')");
					$querydatajum3 = $this->db->query("SELECT * FROM agenda WHERE id_pegawai='$datapegikut' AND tanggal='$tanggal' AND (jam_mulai >= '$jammulai2' AND jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') ");
			...
			if($querydatajumdata>0 || $jumdatapegawai>0)
			{
					 ...
					 {		 $pegawai='';
							 ...
							 foreach($datapegawai as $datapegikut)
							 {
								 $querydatajum2 = $this->db->query("SELECT * FROM agendapegawai a INNER JOIN agenda b ON(a.id_agenda=b.id_agenda) WHERE a.id_pegawai='$datapegikut' AND b.tanggal='$tanggal' AND (b.jam_mulai >= '$jammulai2' AND b.jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') AND b.id_agenda NOT IN($id)");
								
								 foreach($querydatajum2->result_array() as $datapegawai)
								 {
									 $idagenda=$datapegawai['id_agenda'];
									 $querydatapegawaiagenda = $this->db->query("SELECT * FROM agendapegawai WHERE id_agenda='$idagenda'");
						...
						$this->session->set_flashdata('pesan',$pesan);
						redirect(base_url('agenda/agenda_tambah',$x));
			}
			else
			{
				$inputData=array(
				...
				$cek=$this->db->insert('agenda',$inputData);
	  ...
	  $x = array('judul'        => 'Tambah Data Agenda' ,
		...
		if(isset($_POST['kirim']))
		{
		  ...
		  if($cek)
		  {
				...
				$this->session->set_flashdata('pesan',$pesan);
				redirect(base_url('agenda/agenda_kegiatan/'.$idm));
		  }
		  else
		  {
		   echo "ERROR input Data";
  ...
  {
	  $sql=$this->db->get_where('agenda',array('id_agenda'=>$id))->row_array(); 
	  ...
	  $x = array('judul'        =>'Edit Data Agenda' ,
		...
		if(isset($_POST['kirim']))
		{
			...
			$querydatajumdata=0;
			$querydatajum = $this->db->query("SELECT * FROM agenda WHERE id_pegawai='$idpegawai' AND tanggal='$tanggal' AND (jam_mulai >= '$jammulai2' AND jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') AND id_agenda NOT IN($id)");
			$querydatajumdata=$querydatajum->num_rows();
			$querydatajumdatarow=$querydatajum->row();
			
			$querydatajum21= $this->db->query("SELECT * FROM agendapegawai a INNER JOIN agenda b ON(a.id_agenda=b.id_agenda) WHERE a.id_pegawai='$idpegawai' AND b.tanggal='$tanggal' AND (b.jam_mulai >= '$jammulai2' AND b.jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') AND b.id_agenda NOT IN($id)");
			$querydatajumdata21=$querydatajum21->num_rows();
			...
			if(!empty($datapegawai))
			{
				...
				{
					$querydatajum2 = $this->db->query("SELECT * FROM agendapegawai a INNER JOIN agenda b ON(a.id_agenda=b.id_agenda) WHERE a.id_pegawai='$datapegikut' AND b.tanggal='$tanggal' AND (b.jam_mulai >= '$jammulai2' AND b.jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') AND b.id_agenda NOT IN($id)");
					$querydatajum3 = $this->db->query("SELECT * FROM agenda WHERE id_pegawai='$datapegikut' AND tanggal='$tanggal' AND (jam_mulai >= '$jammulai2' AND jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') AND id_agenda NOT IN($id)");
			...
			else
			{
				...
				$cek=$this->db->update('agenda',$inputData,array('id_agenda'=>$id));
  ...
  public function kegiatan_detail($id='')
  {
	   ...
	   tpl('admin/kegiatan_formdetail',$x);	
  }
  ...
  {
	   $cek=$this->db->delete('agenda',array('id_agenda'=>$id));
	   if ($cek) {
		$pesan='<div class="alert alert-success alert-dismissible">
					<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
					<h4><i class="icon fa fa-check"></i> Success!</h4>
  ...
  public function kegiatan_hapus($id='',$idm='')
  {
	...
	public function myexcel()
	{
		...
		$id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
		$data = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($id) : $this->m_admin->agenda_pegawai();
		$data2 = ($this->session->userdata('level') == "pegawai") ? $this->m_admin->cari_agenda_pegawai($idm2) : $this->m_admin->cari_agenda_pegawai($idm2);
