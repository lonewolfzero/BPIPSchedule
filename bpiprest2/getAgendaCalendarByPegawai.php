...
if(!empty($id))
{
	...
	$hasil = $connect->query("SELECT * FROM agenda a LEFT JOIN pegawai b ON(a.id_pegawai=b.id_pegawai) LEFT JOIN deputi c ON(b.id_deputi=c.id_deputi) LEFT JOIN unitkerja d ON(b.id_unitkerja=d.id_unitkerja) WHERE a.id_pegawai='$id' order by a.tanggal desc");
	...
	$hasil3 = $connect->query("SELECT * FROM agendapegawai e INNER JOIN agenda a ON(a.id_agenda=e.id_agenda) LEFT JOIN pegawai b ON(e.id_pegawai=b.id_pegawai) LEFT JOIN deputi c ON(b.id_deputi=c.id_deputi) LEFT JOIN unitkerja d ON(b.id_unitkerja=d.id_unitkerja) WHERE e.id_pegawai='$id' order by a.tanggal desc");
